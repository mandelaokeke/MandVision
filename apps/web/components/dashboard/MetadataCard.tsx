...
export function MetadataCard({
  ...
  const status = stage === "complete" ? "COMPLETE" : progressLabel.toUpperCase();
  ...
  const metadataFileType = metadata && "fileType" in metadata
    ? metadata.fileType
    : undefined;

  const metadataMediaType = metadata && "mediaType" in metadata
    ? metadata.mediaType
    : undefined;

  const metadataExtractionStatus = metadata && "extractionStatus" in metadata
    ? metadata.extractionStatus
    : undefined;

  const metadataWordCount = metadata && "wordCount" in metadata
    ? metadata.wordCount
    : undefined;
  ...
  return (
    <Card className="rounded-2xl border-white/10 bg-[#0d131c] text-white shadow-2xl shadow-black/20">
      <CardHeader className="flex flex-row items-start gap-4 space-y-0">
        ...
        <div>
          <CardTitle className="text-xl">File Metadata</CardTitle>
      ...
      <CardContent>
        <dl className="space-y-4 text-sm">
          ...
          <MetadataRow label="Media Type" value={metadataMediaType || "—"} />
          <MetadataRow label="File Type" value={metadataFileType || "—"} />
          <MetadataRow label="Extraction" value={metadataExtractionStatus || "—"} />
          <MetadataRow
            label="Word Count"
            value={typeof metadataWordCount === "number" ? metadataWordCount.toString() : "—"}
          />
